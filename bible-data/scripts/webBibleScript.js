...
const inputZip = path.join(__dirname, "..", "source", "eng-web_html.zip");
...
// Book mapping with file prefixes and IDs (same as ESV for compatibility)
const BOOK_MAP = {
  // Old Testament
...
function processHtmlFile(filePath, bookId, chapterNumber) {
  ...
  $(".main")
    ...
    .each((i, elem) => {
      ...
      // Section headings (h2, s, s1, s2 classes)
      if (
        tagName === "h2" ||
        $elem.hasClass("s") ||
        $elem.hasClass("s1") ||
      ...
      // Paragraph elements (p, pi, q, q2, q3, etc.)
      if (
        tagName === "div" &&
        ($elem.hasClass("p") ||
          $elem.hasClass("pi") ||
        ...
        // Poetry has minimum indent level of 2 (base for poetry)
        let indentLevel = 0;
        if ($elem.hasClass("q")) indentLevel = 2;   // base poetry level
        if ($elem.hasClass("q2")) indentLevel = 3;  // q2 = one more indent
        if ($elem.hasClass("q3")) indentLevel = 4;  // q3 = two more indents
        if ($elem.hasClass("q4")) indentLevel = 5;  // q4 = three more indents
        if ($elem.hasClass("mi")) indentLevel = 2;  // margin paragraph = base poetry level

        const isPoetry = indentLevel > 0;
        ...
        $content.contents().each((j, node) => {
          if (node.type === "tag" && $(node).hasClass("verse")) {
            ...
            // Accumulate text (skip footnote links)
            if (node.type === "text") {
...
async function main() {
  ...
  try {
    console.log("Starting WEB Bible data extraction...");
