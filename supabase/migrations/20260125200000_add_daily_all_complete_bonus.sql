-- =============================================================================
-- Add Daily All Complete Bonus
-- Awards +20,000 XP when user completes all 5 daily activities
-- (login, reading, notes, devotion, plan)
-- =============================================================================

-- 1. Add reward type to config
INSERT INTO reward_points_config (reward_type, points, description)
VALUES ('daily_all_complete', 20000, 'Complete all 5 daily activities')
ON CONFLICT (reward_type) DO UPDATE SET
  points = EXCLUDED.points,
  description = EXCLUDED.description;

-- 2. Update the record_activity_with_streak function to check for all-complete bonus
...
RETURNS TABLE(
  ...
  total_xp INTEGER,
  level INTEGER,
  all_complete_bonus_xp INTEGER  -- NEW: bonus for completing all 5 activities
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Check for ALL COMPLETE bonus (all 5 activities done today)
  SELECT COUNT(*) INTO v_activities_completed
  FROM user_activity_streaks
  WHERE user_id = p_user_id
    AND activity_type IN ('login', 'reading', 'notes', 'devotion', 'plan')
    AND last_activity_date = v_today;

  IF v_activities_completed = 5 THEN
